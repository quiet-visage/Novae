...
void motion_update_x(Motion* m, float target, float delta_time) {
    ...
    for (size_t i = 0; i < iterations; i++) {
        ...
        m->velocity[0] = m->velocity[0] +
                         delta_time * (target + k3 * estimated_velocity - m->position[0] - k1 * m->velocity[0]) / k2;
    }
}
...
void motion_update_y(Motion* m, float target, float delta_time) {
    ...
    for (size_t i = 0; i < iterations; i++) {
        ...
        m->velocity[1] = m->velocity[1] +
                         delta_time * (target + k3 * estimated_velocity - m->position[1] - k1 * m->velocity[1]) / k2;
    }
}
...
void motion_update(Motion* m, float target[2], float delta_time) {
    ...
    float estimated_velocity[2] = {target[0] - m->previous_input[0], target[1] - m->previous_input[1]};
    ...
    for (size_t i = 0; i < iterations; i++) {
        ...
        m->velocity[0] =
            m->velocity[0] +
            delta_time * (target[0] + k3 * estimated_velocity[0] - m->position[0] - k1 * m->velocity[0]) / k2;
        m->velocity[1] =
            m->velocity[1] +
            delta_time * (target[1] + k3 * estimated_velocity[1] - m->position[1] - k1 * m->velocity[1]) / k2;
    }
}
...
void motion_hard_set(Motion* m, float x, float y) {
    m->position[0] = x;
    m->position[1] = y;
    m->previous_input[0] = x;
    m->previous_input[1] = y;
}
