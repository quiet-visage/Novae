...
inline float task_height(void) { return g_cfg.inner_gap * 2 + g_style->typo.size; }

Task_Return_Flags task_draw(Task* m, float x, float y, float max_width) {
#define CHECK_BTN_RAD 16.0f
    Task_Return_Flags result = 0;
    Rectangle bg;
    bg.x = x + g_cfg.btn_pad_horz * 2 + BTN_ICON_SIZE + g_cfg.inner_gap;
    bg.y = y;
    bg.height = g_cfg.inner_gap * 2 + g_style->typo.size;
    ...
    float name_y = y + bg.height * .5f - g_style->typo.size * .5f;

    float move_up_btn_width = btn_width(&m->move_up_btn) + BTN_ICON_SIZE;
    bg.width = max_width - g_cfg.inner_gap - move_up_btn_width;

    float rad = bg.width > bg.height ? 2 * 8 / bg.height : 2 * 8 / bg.width;
    Color col = GET_RCOLOR(COLOR_BASE);
    ...
    bool fpo = m->complete;
    bool clicked_check =
        check_btn_draw(&m->check_btn, &fpo, check_btn_x, check_btn_y, DRAW_ENABLE_MOUSE_INPUT);
    ...
    if (m->left) {
        ...
        float range_str_w = ff_measure_utf32(range_str, range_str_len, *g_style).width;
        float range_str_x = (cx + bg.width) - range_str_w - (g_cfg.outer_gap + g_cfg.inner_gap);
        ff_draw_str32(range_str, range_str_len, range_str_x, name_y,
                      (float*)g_cfg.global_projection, *g_style);
    } else {
        float icon_x = (x + bg.width) - BTN_ICON_SIZE - g_cfg.outer_gap;
    ...
    Rectangle move_up_btn_bg = {
        .x = x, .y = y, .width = BTN_ICON_SIZE + g_cfg.btn_pad_horz * 2, .height = bg.height};
    rad = move_up_btn_bg.width > move_up_btn_bg.height ? 2 * 8 / move_up_btn_bg.height
                                                       : 2 * 8 / move_up_btn_bg.width;
    DrawRectangleRounded(move_up_btn_bg, rad, 16, col);
    bool clicked_move_up = btn_draw_with_icon(&m->move_up_btn, ICON_MOVE_UP, x,
