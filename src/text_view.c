...
void horizontal_scroll(const C32* buf, size_t len, size_t curs,
    ...
    {  // right horizontal scroll
        ...
        float w = ff_measure_utf32(buf, column, font, size, 0).width;
    ...
    {  // left horizontal scroll
        ...
        float w =
            ff_measure_utf32(buf, column, font, size, true).width;
...
void get_text_projection(float out[4][4], Motion* mo) {
    if (mo) {
        ...
        float top = 0;
        ff_get_ortho_projection(left, right, bottom, top, -1.f, 1.f,
                                out);
        return;
    } else
...
inline void update_scroll_animation(Motion* motion,
                                    float target_motion_x,
...
float get_cursor_x(const C32* buf, size_t len, FF_Typo typo,
    ...
    float cursor_offset = ff_measure_utf32(buf_cursor_ptr, cursor_pos,
                                           typo.font, typo.size, 0)
                              .width;
