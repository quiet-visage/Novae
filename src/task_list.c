...
inline void task_list_push(Task_List* m, Task t) { task_vec_push(&m->tasks, t); }
...
Changed_Task task_list_draw(Task_List* m, float x, float y, float max_w, float max_h,
    ...
    for (size_t i = 0; i < m->tasks.len; i += 1, task_y += task_h + g_cfg.inner_gap) {
        if ((task_y + task_h) < y) continue;
        if ((task_y - y) >= max_h) break;
        Task* task = &m->tasks.data[i];
        if (db_is_default_task(task->db_id)) continue;
